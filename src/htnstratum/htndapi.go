...
func (ks *KaspaApi) startStatsThread(ctx context.Context) {
	...
	for {
		...
		case <-ctx.Done():
			...
			return
		case <-ticker.C:
			dagResponse, err := ks.htnd.GetBlockDAGInfo()
			if err != nil {
				ks.logger.Warn("failed to get network hashrate from kaspa, prom stats will be out of date", zap.Error(err))
				continue
			}
			response, err := ks.htnd.EstimateNetworkHashesPerSecond(dagResponse.TipHashes[0], 1000)
			if err != nil {
				ks.logger.Warn("failed to get network hashrate from kaspa, prom stats will be out of date", zap.Error(err))
				continue
			}
...
func (ks *KaspaApi) GetBlockTemplate(
	...
	template, err := ks.htnd.GetBlockTemplate(client.WalletAddr,
